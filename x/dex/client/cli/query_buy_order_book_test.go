...
import (
	...
	"github.com/cristinaITdeveloper/testchange/testutil/network"
	"github.com/cristinaITdeveloper/testchange/x/dex/client/cli"
...
func networkWithBuyOrderBookObjects(t *testing.T, n int) (*network.Network, []types.BuyOrderBook) {
	...
	for i := 0; i < n; i++ {
		state.BuyOrderBookList = append(state.BuyOrderBookList, types.BuyOrderBook{
...
func TestListBuyOrderBook(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.Subset(t, objs, resp.BuyOrderBook)
		}
	})
